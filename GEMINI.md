...
### 4. Standing Orders (Applies to ALL AI Agents)

These are non-negotiable rules. Every AI agent (Claude, Gemini, Copilot, Jules) MUST follow them:

1. **Never execute database migrations directly.** Always generate `.sql` files for manual review. No `drizzle-kit push`, no `prisma migrate`, no raw DDL unless explicitly approved by the user.
2. **Never use `--force`, `--yes`, `-y`, or any bypass flags in CLI commands.** If a tool asks for confirmation — stop and ask the user.
3. **Never put API keys, passwords, or secrets in code, commits, or logs.** If a command outputs a secret, warn the user first.
4. **Never install packages without explicit approval.** Typosquatting is real. Always confirm the exact package name with the user before `npm install` / `pnpm add`.
5. **Never run destructive commands** (`rm -rf`, `DROP TABLE`, `TRUNCATE`, `git push --force`, `docker system prune`) without triple-confirmed explicit consent.
6. **Never commit, push, or deploy without explicit user request.** Write code, don't touch the pipeline without permission.
7. **Never read `.env`, `.env.*`, `~/.ssh/`, or files containing secrets** unless the user explicitly asks and confirms.

---

### 🎯 Task-to-Agent & Skill Mapping

Перед началом любой задачи определи её домен и активируй соответствующего агента и набор скиллов. **Это закон.**

| Домен (Domain)              | Агент (Agent)            | Ключевые скиллы (Primary Skills)                              |
| :-------------------------- | :----------------------- | :------------------------------------------------------------ |
| **Frontend / UI / UX**      | `frontend-specialist.md` | `frontend-design`, `react-patterns`, `tailwind-patterns`      |
| **Backend / API / DB**      | `backend-specialist.md`  | `api-patterns`, `database-design`, `nestjs-expert`            |
| **DevOps / Deploy / SRE**   | `devops-engineer.md`     | `deployment-procedures`, `server-management`, `docker-expert` |
| **Mobile / Cross-platform** | `mobile-developer.md`    | `mobile-design`                                               |
| **Architecture / Planning** | `project-planner.md`     | `architecture`, `plan-writing`                                |
| **Security / Audit**        | `security-auditor.md`    | `vulnerability-scanner`, `red-team-tactics`                   |
| **Debugging / Legacy**      | `debugger.md`            | `systematic-debugging`, `clean-code`                          |
| **Systems / Performance**   | `backend-specialist.md`  | `systems-programming`, `performance-profiling`                |

### 🚀 Протокол маршрутизации (Routing)

1. **Анализ**: Пойми, куда текут данные (Фронт? Бэк? Прод?).
2. **Активация**: Прочитай файл агента в `.agent/agents/{agent}.md`.
3. **Загрузка**: Проверь список `skills:` в агенте и подтяни соответствующие `SKILL.md`.
4. **Следование**: Используй воркфлоу, прописанный в этих файлах (например, "Deep Design Thinking" для фронта).
